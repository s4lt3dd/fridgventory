...
const CONFIG: Record<UrgencyLevel, { title: string; tone: 'danger' | 'warning' | 'safe' }> = {
    ...
    <section className="mb-6">
      ...
      >
        <div className="flex items-center gap-2.5">
          <span className={clsx('h-2.5 w-2.5 rounded-full', toneDot[config.tone])} />
          <h2 className={clsx('font-display text-2xl', toneStyles[config.tone])}>
            {config.title}
          </h2>
