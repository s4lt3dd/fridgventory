...
export default function SettingsPage() {
  ...
  const [days, setDays] = useState<Record<number, boolean>>({ 1: true, 3: true, 7: false });

  return (
    <div className="mx-auto max-w-2xl space-y-6">
      <div>
        <h1 className="font-display text-4xl sm:text-5xl leading-none text-text-primary">Settings</h1>
        <p className="mt-1 text-sm text-text-muted">Tweak FridgeCheck to work how you want it to.</p>
      </div>
      ...
      <Card>
        ...
        <dl className="space-y-2 text-sm">
          <div className="flex justify-between gap-4">
            ...
            <dd className="font-semibold text-text-primary">{user?.username}</dd>
          </div>
          <div className="flex justify-between gap-4">
            <dt className="shrink-0 text-text-muted">Email</dt>
            <dd className="min-w-0 truncate font-semibold text-text-primary">{user?.email}</dd>
          </div>
        </dl>
      </Card>
      ...
      <Card>
        ...
        <div className="space-y-4">
          <Toggle checked={emailEnabled} onChange={setEmailEnabled} label="Email reminders" />
          <div>
            <p className="mb-2 text-sm text-text-primary">Remind me before expiry</p>
      ...
      {/* Danger zone */}
      <Card>
        <h2 className="mb-3 text-sm font-semibold uppercase tracking-wide text-text-muted">Account</h2>
